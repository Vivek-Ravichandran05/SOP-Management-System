...
import { jwtDecode } from "jwt-decode";                                  /* importing the decoding library*/
...
function Dashboard () {
    ...
    useEffect (() => {
        ...
        try{
            const decode = jwtDecode(token);                             /*the decoded jwt token stored in decode*/
            setName(decode.name);                                        /*from the decode we take name and store it*/
            fetchSopCount();
        }
        catch(err){
    ...
    const fetchSopCount = async() => {                                   /* this function fetches the sop count by calling the respective API */
        try {
            const coun = await api.get("/sops/count");                   
            setsopCount(coun.data.Total);                                /* the total info is called using this */
        }
        catch(err) {
            console.log(err);
        }
    ...
    const stats = [{ label:"Total SOPs",value:sopCount, icon: "📄" }];
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
                            ...
                            <div className="flex items-center gap-4">
                                <span className="hidden sm:block text-sm text-slate-400">
                                    Welcome, <span className="text-amber-400 font-medium">{name}</span>
                                </span>
                                <button onClick={handlelogout} className="px-4 py-2 text-sm font-medium rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/20 hover:from-amber-400 hover:to-orange-400 hover:shadow-amber-500/30 active:scale-[0.97] transition-all duration-300">
                                    Logout
                                </button>
                            </div>
                        </div>
                    ...
                    <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                      ...
                      <div className="bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8 shadow-2xl shadow-black/20">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            <button onClick={() => navigate("/createsop")} className="px-6 py-3 font-semibold rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/20 hover:from-amber-400 hover:to-orange-400 hover:shadow-amber-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                                + Create SOP
                            </button>
                            <button onClick={() => navigate("/allsops")} className="px-6 py-3 font-semibold rounded-xl bg-slate-900/50 border border-slate-600/50 text-slate-300 hover:bg-slate-700/50 hover:text-white hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                                View All SOPs
                            </button>    
                        </div>
                      </div>
